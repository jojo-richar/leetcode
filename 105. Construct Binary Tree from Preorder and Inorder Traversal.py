...
class Solution:
    def buildTree(self, preorder, inorder) :
        def buildmytree(preorder_left, preorder_right, inorder_left, inorder_right):
            ...
            inorder_root = inorder.index(preorder[preorder_left])
            size_left_subtree = inorder_root - inorder_left

            root = TreeNode(preorder[preorder_left])
            root.left = buildmytree(preorder_root + 1, preorder_root + size_left_subtree, inorder_left,
                                    inorder_left + size_left_subtree)
            root.right = buildmytree(preorder_left + size_left_subtree + 1, preorder_right, inorder_root + 1,
                                     inorder_right)
            return root

        n = len(preorder)
        index = {element: i for i, element in enumerate(inorder)}
        buildmytree(0, n - 1, 0, n - 1)
